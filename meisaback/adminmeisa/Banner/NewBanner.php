...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-meisa-red">Dashboard for Banner</h1>
                    ...
                    <?php 
                        include '../assets/components/backend/conexion.php';
                        $sql = "SELECT * FROM banners ORDER BY prioridad"; 
                        $query = $mbd -> prepare($sql); 
                        $query -> execute(); 
                        $results = $query -> fetchAll(PDO::FETCH_OBJ); 
                        if($query -> rowCount() > 0){ 
                            foreach($results as $result) { 
                                $datos=$result -> id_banner."||".
                                        $result -> img."||".
                                        $result -> prioridad."||".
                                        $result -> title."||". 
                                        $result -> seo; ?>
                            <div class="card" style="width: 18rem; margin: 1rem;">
                                ...
                                <div class="card-body bg-meisa-dark">
                                    <h5 class="card-title text-white"><?php echo $result -> title; ?></h5>
                                    ...
                                    <a href="http://localhost/meisa/meisaback/adminmeisa/assets/<?php echo $result -> img; ?>" class="card-text text-white" target="_blank">
                                        URL: <?php echo $result -> id_banner; ?>
                                    </a>
                                </div>
                                <div class="card-body bg-meisa-dark">
                                    <button class="btn btn-link text-warning col-5" type="button" data-toggle="modal" data-target="#modalBannerEditar" onclick="verEdiatarBanner('<?php echo $datos ?>')"><i class="fas fa-edit"></i> Editar</button>
                                    <button class="btn btn-link text-danger col-5" type="button" onclick="preguntarSiNoBanner('<?php echo $result -> id_banner; ?>')"><i class="fas fa-trash"></i> Eliminar</button>
                                </div>
                            </div>
                            <?php 
                            }
    ...
    <div class="modal fade" id="modalBanner" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-header bg-meisaa">
                <h5 class="modal-title text-light" id="exampleModalLabel">Nuevo Banner</h5>
            ...
            <div class="modal-footer">
                <button type="submit" id="btnAddBanner" class="btn btn-meisa">Guardar</button>
            </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="modalBannerEditar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-body">
                <form class="formBannerEdit" id="formBannerEdit"  method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-footer">
                <button type="button" id="btnEditBanner" class="btn btn-success">Guardar</button>
            </div>
            </div>
        </div>
    </div>
