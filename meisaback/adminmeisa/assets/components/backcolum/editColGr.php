...
if($img["type"] == "image/jpeg" or $img["type"] == "image/png"){
    ...
    $consulta = "UPDATE columnfour SET img = '$rutaimg', prioridad = '$priority', title = '$title', content = '$content', engtitle = '$titleeng', engcont = '$contenteng' WHERE id_col = '$idcoll' "; 
    $eject = $mbd -> prepare($consulta); 
    $eject -> execute(); 
    echo 1;
}else{
    $consulta = "UPDATE columnfour SET prioridad = '$priority' WHERE id_col = '$idcoll' "; 
    $eject = $mbd -> prepare($consulta); 
    $eject -> execute();
    echo 1; 
}